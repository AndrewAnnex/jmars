...
public class LManager2 extends LManager implements LViewFactory.Callback
 {
    ...
    public void receiveNewLView(Layer.LView view) {
		log.println(view);
		dockTabs.addTab(view.getName(), view.light2, view.getFocusPanel(), null);
		viewList.add(view);
    ...
    public void updateLabels() {
		for (int i = 0; i < mainPanel.rows.size(); i++) {
			...
			r.label.setText(" " + r.view.getName());
			r.label.setToolTipText(" " + r.view.getName());
			dockTabs.setTitleAt(mainPanel.rows.size() - i, r.view.getName());
		}
		rebuildMenus();
	}
...
//	mainPanel.repaint();
	...
	getContentPane().add(dockTabs.getMainTabbedPane(),
			     BorderLayout.CENTER);
     }
		...
		for(int i=0; i<rows.size(); i++)
		 {
		    ...
		    int y;
		    if     (i == dragSrc                  ) y = dragPos;
		    else if(i >  dragSrc  &&  i <= dragDst) y = h * (i-1);
		    else if(i <  dragSrc  &&  i >= dragDst) y = h * (i+1);
		    else                                    y = h *  i   ;
		    r.setSize(w, h);
	...
	void rebuildAddMenu() {
		...
		MultiFactory.addAllToMenu(LManager2.this,
			menu, LViewFactory.factoryList2);
	...
	MainPanel()
	 {
	    ...
	    Dimension d = new Dimension(Math.max(d1.width, d2.width),
	...
	Action actDelete =
	new AbstractAction("Delete layer") {
			public void actionPerformed(ActionEvent e) {
	...
	void setDragOffset(int dragOffset)
	 {
		...
		dragPos = Util.bound(0,
				     h * dragSrc + dragOffset,
				     h * (rows.size()-1));
		dragDst = (dragPos + h/2 - 1) / h;
	     }
	 }

	MouseInputListener rowMouseHandler
    ...
    private static final Color FOCUS_COLOR = UIManager.getColor(
	"ToggleButton.focus");

    private static final boolean DARK_DRAG = Config.get(
	"lmanager.darkdrag", false);

    private static final Color darkColor = UIManager.getColor(
	"ToggleButton.select");
    private final Color lightColor = getBackground();

    private static Border selectedBorder = new SoftBevelBorder(
	Config.get("lmanager.raised", true)
	? BevelBorder.RAISED
	: BevelBorder.LOWERED);
    private static Border unselectedBorder = new EmptyBorder(selectedBorder.getBorderInsets(null));
    private static Border selectedBorder2 = new LineBorder(
	Color.blue, //FOCUS_COLOR,
	...
	Row(final Layer.LView view, MouseInputListener rowMouseHandler)
	 {
	    ...
	    this.label = new JLabel(" " + view.getName());
	    this.slider = new JSlider(0, 1000, (int)(view.getAlpha()*1000));
...
//	    btn.addMouseListener(hiliteHandler);
    ...
    private ActionListener visibilityHandler =
    new ActionListener()
     {
	public void actionPerformed(ActionEvent e)
	 {
	...
	MouseListener tabDoubleClickHandler =
	new MouseAdapter()
	 {
	    public void mouseClicked(MouseEvent e)
	     {
    ...
    // Should probably go in edu.asu.jmars.swing!
    private static class HiddenToggleButton extends JToggleButton
     {
	public static final Color DEFAULT_BACK
	= UIManager.getColor("Button.background");

	public static final Border borderNone
	= BorderFactory.createLineBorder(DEFAULT_BACK, 2);

	public static final Border borderRaised
...
//	= BorderFactory.createRaisedBevelBorder();
	...
	public static final Border borderLowered
	= BorderFactory.createCompoundBorder(
	    borderNone,
	    new LineBorder(Color.black, 1));
...
//	BorderFactory.createLoweredBevelBorder();
	...
	private static final ActionListener borderChanger =
	new ActionListener()
	 {
	    public void actionPerformed(ActionEvent e)
	     {
	...
	private static final MouseListener borderActivator =
	new MouseAdapter()
	 {
    ...
    private MouseListener hiliteHandler =
    new MouseAdapter()
     {
	Border hilited = new SoftBevelBorder(BevelBorder.RAISED);
	Border pressed = new SoftBevelBorder(BevelBorder.LOWERED);
	Border unhilited = BorderFactory.createLineBorder(getBackground(), hilited.getBorderInsets(null).top);
//	Border unhilited = new EmptyBorder(hilited.getBorderInsets(null));

	public void mouseEntered (MouseEvent e) { setBorder(e,   hilited); }
	public void mousePressed (MouseEvent e) { setBorder(e,   pressed); }
	public void mouseReleased(MouseEvent e) { setBorder(e,   hilited); }
	public void mouseExited  (MouseEvent e) { setBorder(e, unhilited); }
	...
	for(int i=1, viewIdx=0;
	    i<states.length && viewIdx<viewList.size();
	    i++, viewIdx++)
	 {
