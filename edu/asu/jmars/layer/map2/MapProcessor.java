...
public final class MapProcessor implements Runnable {
	...
	public MapProcessor(MapRequest request, Pipeline pipe, int pipelineIndex, MapChannel channel) {
		this.myRetriever = MapRetriever.getMapData(request);
		this.pipe = pipe;
		this.pipeIndex = pipelineIndex;
		this.destination = channel;
		myRetriever.setReceiver(this);
	}
	...
	public void receiveUpdate() {
		if (requestFinished) {
			...
			// If  there is no image or Pipeline, send updates to the channel immediately
			destination.receiveMap(myRetriever.getData(true));
		} else {
			pendingUpdate = true;
			queueProcessing();
		}
	...
	public void run() {
		...
		MapData result = retrievedData = myRetriever.getData(true);
		...
		for (int i = 0; i < stages.length; i++) {
			try {
				if (i == (stages.length-1)){
					synchronized(stages[i]){
						...
						if (stages[i].getInputCount() > 1 && result != null)
							result = result.getDeepCopy(false);
					}
				}
				else {
				...
				log.aprintln("Error in processing stage " + stages[i].getClass().getName() + ": " + e.getMessage());
				log.println(e);
				log.printStack(10);
				result = null;
				break;
		...
		destination.receiveMap(result == null ? retrievedData.getDeepCopyShell(null) : result);
