...
public class NomenclatureLView extends Layer.LView
 {
    ...
    private ArrayList landmarks = new ArrayList();
    private Hashtable landmarkTypes = new Hashtable();

    private Hashtable labelLocations = new Hashtable();
    ...
    public class MarsFeature {
      ...
      public Point2D thisWorldPoint = new Point2D.Double(0,0); //do not assume this to be the same value always
      ...
      public String getPopupInfo(boolean showAsHTML) {
        ...
        try {
          ...
          info += " (" + f.format((360-longitude)%360) + "E, " + f.format(latitude) + "N, " + f.format(this.diameter) + "km )";
...
//         thisWorldPoint = getProj().spatial.toWorld(longitude, latitude);
...
//       }
      ...
      protected void drawPoint(Graphics2D g2, Point2D worldPoint)
      {
        ...
        Rectangle2D.Double box = new Rectangle2D.Double(worldPoint.getX() - pSize.getWidth()*2, worldPoint.getY()+pSize.getHeight()*2,pSize.getWidth()*4, pSize.getHeight()*4);
        g2.fill(box);
      ...
      protected boolean inConflict( Rectangle2D r) {
        ...
        for (Enumeration e = labelLocations.elements() ; e.hasMoreElements() ;) {

          Rectangle2D existingRect = (Rectangle2D) e.nextElement();
...
//       {
//           drawLabel(g2, this.thisWorldPoint);
//       }
      ...
      protected void drawLabel(Graphics2D g1, Graphics2D g2, Point2D pt)
      {
        ...
        //calculate new label location and see if it intersects with one already.

        double xWidth = fontMetrics.stringWidth(name)*getProj().getPixelSize().getWidth()*1.2; //some magic number
        double xLoc =  pt.getX()-xWidth/2;

        double yHeight = fontMetrics.getHeight()*getProj().getPixelSize().getHeight();
        double yLoc = pt.getY()-yHeight*.7; //we want to include the point in the bounds

        Rectangle2D.Double labelLoc = new Rectangle2D.Double(xLoc, yLoc, xWidth, yHeight);
        if ( inConflict(labelLoc) ) {
          //try some different locations if there is a conflict with other labels or points
...
//               if ( inConflict(labelLoc) ) {
//                 xLoc = pt.getX() + getProj().getPixelSize().getWidth()*15;
//                 yLoc = pt.getY();
//                 labelLoc = new Rectangle2D.Double(xLoc, yLoc, xWidth, yHeight);
//               }
...
//             }
        ...
        //store the labels location - ok to overwrite point locations since they are inclusive
    ...
    public NomenclatureLView(Layer parent)
    {
        ...
        /**
         * Read in the file of landmarks on mars - a file was
         * chosen instead of a db table since the data is fairly static
         * and it will make it a hell of a lot easier for the student
         * version - which is supposed to operate without network connectivity.
         *
         * The format of the file is comma separated and should contain the
         * following fields:
         *
         * lanmark type, name, latitude(N), longitude (W), diameter (km), origin/description
		 *
          ...
          try {
            ...
            while ( lineIn != null && lineIn.compareToIgnoreCase("STOP") != 0 ) {
              ...
              try {
                ...
                if ( ! landmarkTypes.containsKey(mf.landmarkType) )
                  landmarkTypes.put(mf.landmarkType, new Boolean(true));
    ...
    protected Component[] getContextMenuTop(Point2D worldPt)
    {
      ...
      for ( int i=0 ; i < menuItems.length; i++ )
        descriptionItems[i+1] = menuItems[i]; /* X-fer old items into our new holder*/
    ...
    protected Component[] getContextMenu(Point2D worldPt) {
      ...
      for (int i=0 ; i < menuItems.length; i++ ) {
        newMenuItems[i] = menuItems[i]; /* X-fer old items into our new holder*/
      }

      newMenuItems[menuItems.length] = new JMenuItem(new AbstractAction("Copy Nomenclature Description to Clipboard") {
    ...
    protected String getStringForPoint(Point2D worldPoint, boolean showAsHTML) {
      ...
      for (Enumeration e = labelLocations.keys() ; e.hasMoreElements() ;) {

        MarsFeature mf = (MarsFeature) e.nextElement();
        Rectangle2D r = (Rectangle2D)  labelLocations.get(mf);

        if ( ! settings.showLandmarkTypes.contains(mf.landmarkType) )
          continue;

        if ( r != null && r.contains(worldPoint) || r.contains(new Point2D.Double(worldPoint.getX()-360.0, worldPoint.getY())) )  {
    ...
    private static class NomenclatureSettings extends LViewSettings
    {
      ...
      ArrayList showLandmarkTypes = new ArrayList();
    ...
    */
    ...
    private class NomenclaturePanel extends JPanel implements ActionListener, ListSelectionListener
    {
          ...
          NomenclaturePanel()
           {
             ...
             JPanel p1 = new JPanel(new GridLayout(3,0));
              ...
              p2.setBorder(
                BorderFactory.createCompoundBorder(
                        BorderFactory.createTitledBorder("Tooltips:"),
                        BorderFactory.createEmptyBorder(5, 5, 5, 5)
                        )
                );
              box.add(p2);
             ...
             p3.setBorder(
                BorderFactory.createCompoundBorder(
                        BorderFactory.createTitledBorder("Selected Landmarks:"),
                        BorderFactory.createEmptyBorder(5, 5, 5, 5)
                        )
                );
              ...
              for (Enumeration e = landmarkTypes.keys() ; e.hasMoreElements() ;) {
             ...
             listLandmarks.setToolTipText("This list indicates which landmarks appear. Use the shift and control keys with the mouse to make multiple selections.");
             ...
             panelX.add(btnClear);
              ...
              btnGoto.setToolTipText("Select a landmark type and then specific landmark, then click here to goto that landmark.");
              p4.add(btnGoto);
          ...
          protected void updateCombo() {
             ...
             for ( int i=0; i < settings.showLandmarkTypes.size() ; i++ ) {
              String type = (String) settings.showLandmarkTypes.get(i);
          ...
          public void valueChanged(ListSelectionEvent e) {
            ...
            Object[] selVals = ( Object[] )listLandmarks.getSelectedValues();
              ...
              else if(source == btnLabelColor)
              {
               ...
               else if(source == comboLandmarks)
               {
                  ...
                  for ( int i=0; i < landmarks.size(); i++ ) {
                        ...
                        if ( mf.landmarkType.compareTo(comboLandmarks.getSelectedItem().toString()) == 0 )
               ...
               else if(source == comboItems)
               {
                  ...
                  for ( int i=0; i < landmarks.size(); i++ ) {
                        ...
                        if ( mf.landmarkType.compareTo(comboLandmarks.getSelectedItem().toString()) == 0 &&
                              mf.name.compareTo(comboItems.getSelectedItem().toString()) == 0) {

						   // JMARS west lon => USER east lon
                          comboItems.setToolTipText(" (" + (360-mf.longitude)%360 + "E, " + mf.latitude + "N)");
                          break;
                        }
                  }
               ...
               else if(source == btnGoto)
               {
                  ...
                  for ( int i=0; i < landmarks.size(); i++ ) {
                        ...
                        if ( Main.inTimeProjection() ) {
                          ...
                          Rectangle2D r = getProj().getWorldWindow();
                          //we want to expand the range of this search to include
                          //the next 100 orbits
                          double l = 100 * 7200.0;
                          Rectangle2D.Double ww = new Rectangle2D.Double(r.getX(), r.getY(), l, r.getHeight());

                          spatialG2 = new SpatialGraphicsSpOb(g2, getProj(), ww);
                        } else
                          spatialG2 = new SpatialGraphicsCyl(g2, getProj());

                        Point2D[] worldPoints = spatialG2.spatialToWorlds(new Point2D.Double(mf.longitude, mf.latitude));
                        if ( worldPoints != null && worldPoints.length > 0  )
    ...
    /////// Utility function used to create the mars_feature page using data from the USGS Momenclature web site -a
