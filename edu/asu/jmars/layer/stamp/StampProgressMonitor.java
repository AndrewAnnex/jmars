...
public class StampProgressMonitor
 {
     ...
     public StampProgressMonitor(Component parentComponent)
     {
	 this(parentComponent, "Projecting image data...",
	      "", 0, 1);
     }

     public StampProgressMonitor(Component parentComponent, Object message,
				 String note,
				 int min,
				 int max)
     {
	 this(parentComponent, message, note, min, max, 0, 0, 0);
     }
	 ...
	 pm = new ProgressMonitor(parentComponent, message, note, min, max,
				  lastDisp, millisToDecideToPopup, millisToPopup,
				  null);
     }

        // Note: use of setProgress may not be compatible with other methods
        // Just added this to be able to use enclosed modal version of ProgressMonitor
	...
	public void incrementStamp(String filename)
	 {
		if(pm != null)
		 {
			currStamp = filename;
	...
	private void updateProgress()
	 {
		pm.setProgress(currTotalTime);
		pm.setNote(
			"Frame " + currTotalTime + "/" + totalTime +
			" (" + currStamp + ")"
			);
	 }
...
/** 
 ** COPY OF SUN'S PROGRESSMONITOR CLASS WITH ONE SINGLE CHANGE: THE
 ** DIALOG IS MODAL.
 ** <hr>
 **
 **
 ...
 * @version 1.22 02/02/00 */
	public class ProgressMonitor extends Object
	 {
	        private static final int DEFAULT_MILLIS_TO_DECIDE_TO_POPUP = 500;
		...
		private long            T0;
		private int             millisToDecideToPopup = DEFAULT_MILLIS_TO_DECIDE_TO_POPUP;
		...
		public ProgressMonitor(Component parentComponent,
		   ...
		   this(parentComponent, message, note, min, max, 
			DEFAULT_MILLIS_TO_DECIDE_TO_POPUP, DEFAULT_MILLIS_TO_POPUP);
		}
	        ...
	        public ProgressMonitor(Component parentComponent,
				       ...
				       int millisToPopup)
	        {
		    this(parentComponent, message, note, min, max, 0, 
			 millisToDecideToPopup, millisToPopup, null);
		}
